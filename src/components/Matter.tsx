...
export const Matter = memo(() => {
  ...
  useEffect(() => {
    ...
    createPill('/pills/orange/about.png', scale, abortControllerRef.current.signal, window.innerHeight - 100);
      ...
      gsap
        ...
        .add(() => {
          if (!aboutCalled.current) {
            loadStaggerPills(
              ['/pills/hello.png', '/pills/keepScrolling.png', '/pills/circle.png'],
              scale,
        ...
        .add(() => {
          if (!oneCalled.current) {
            loadStaggerPills(
              ['/pills/alanJiang.png', '/pills/swe.png', '/pills/age.png'],
              scale,
              abortControllerRef.current?.signal
            );
          }
        ...
        .add(() => {
          if (!twoCalled.current) {
            loadStaggerPills(
              ['/pills/virginiaTech.png', '/pills/circle.png', '/pills/yoe.png'],
              scale,
              abortControllerRef.current?.signal
            );
          }
        ...
        .add(() => {
          if (!threeCalled.current) {
            loadStaggerPills(
              ['/pills/backEnd.png', '/pills/circle.png', '/pills/frontEnd.png'],
              scale,
              abortControllerRef.current?.signal
            );
          }
        ...
        .add(() => {
          if (!fourCalled.current) {
            loadStaggerPills(
              ['/pills/circle.png', '/pills/cloud.png', '/pills/solutionsArchitect.png'],
              scale,
              abortControllerRef.current?.signal
            );
          }
        ...
        .add(() => {
          if (!fiveCalled.current) {
            loadStaggerPills(
              ['/pills/freshIdeas.png', '/pills/passionate.png', '/pills/circle.png'],
              scale,
              abortControllerRef.current?.signal
            );
          }
        ...
        .add(() => {
          if (!sixCalled.current) {
            loadStaggerPills(
              ['/pills/memorableExperiences.png', '/pills/developer.png'],
              scale,
              abortControllerRef.current?.signal
            );
          }
